...
namespace Analytics.Helper
{
    public class Helper
    {
        public static IoTReading Parse(string input)
        {
            ...
            if (!double.TryParse(fields[2], NumberStyles.Float, CultureInfo.InvariantCulture, out double co) ||
                !double.TryParse(fields[3], NumberStyles.Float, CultureInfo.InvariantCulture, out double humidity) ||
                !bool.TryParse(fields[4], out bool light) ||
                !double.TryParse(fields[5], NumberStyles.Float, CultureInfo.InvariantCulture, out double lpg) ||
                !bool.TryParse(fields[6], out bool motion) ||
                !double.TryParse(fields[7], NumberStyles.Float, CultureInfo.InvariantCulture, out double smoke) ||
                !double.TryParse(fields[8], NumberStyles.Float, CultureInfo.InvariantCulture, out double temp))
            {
